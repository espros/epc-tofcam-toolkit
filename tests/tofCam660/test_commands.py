...
def NotImplementedError_Helper(fw_version: str, capability: str, exception_message: str):
    ...
    assert tofCam._version >= fw_version, f"This firmware version is actually capable of {capability}"
...
class Test_setting_calls:
    @pytest.mark.parametrize('integration_times', [[12, 34, 56, 78],
                                                   [25, 16, 0, 0]])
    def test_get_integration_time(self, tofCam: TOFcam660, integration_times: list[int]):
        tofCam.settings.set_integration_hdr(integration_times)
        read = tofCam.settings.get_integration_time()
    ...
    def test_set_illuminator_segments(self, tofCam: TOFcam660):
        try:
            tofCam.settings.set_illuminator_segments(segment_1_on=True, segment_2_on=True, segment_3_on=True, segment_4_on=True)
...
class Test_device_calls:
    def test_get_calibration_data(self, tofCam: TOFcam660):
        try:
            calibData = tofCam.device.get_calibration_data()
    ...
    def test_set_and_get_data_transfer_protocol(self, tofCam: TOFcam660):
        protocols = ["UDP", "TCP", "UDP"]
        for proto in protocols:
            try:
                tofCam.device.set_data_transfer_protocol(proto)
                assert tofCam.device.get_data_transfer_protocol() == proto, "Protocol has not been set"
            except NotImplementedError as e:
...
class Test_flexMod_calls:
    ...
    def test_get_image_functions(self, tofCam: TOFcam660, capture_func):
        # set to FlexMod
        try:
            tofCam.settings.set_flex_mod_freq(12)
            assert tofCam.settings.flexMod == True, "FlexMod has not been enabled"
            assert tofCam._calibData24Mhz is not None, "24MHz modulation frequency calibration unavailable"
        except RuntimeError as e:
            # in case setting to FlexMod is not allowed, we'll get a RuntimeError with error code 8 (camera response).
            assert tofCam.settings.flexMod == False, "FlexMod has been enabled"
            assert tofCam._calibData24Mhz is None, "24MHz modulation frequency calibration would have been available"
        ...
        try:
            ...
            image = getattr(tofCam, capture_func)()
